...
채널 정보(프로필 이미지 포함)를 조회합니다.  
**DB에서 직접 조회하여 응답 속도가 빠릅니다.**

**Query Parameters**:
| 파라미터 | 설명 |
|---------|------|
| channelId | 단일 채널 조회 |
| channelIds | 쉼표 구분 배치 조회 (최대 100개) |

**단일 조회 예시**: `/api/holo/channels?channelId=UC...`
...
```json
{
  ...
  "channel": {
    "id": "UC...",
    "name": "멤버명",
    "photo": "https://yt3.ggpht.com/...=s1024..."
  }
}
```

**배치 조회 예시**: `/api/holo/channels?channelIds=UC1,UC2,UC3`
...
> **Note**: `photo`는 고화질(1024x1024) YouTube 프로필 이미지 URL입니다.  
> DB에서 조회하므로 Holodex API 호출 없이 빠르게 응답합니다.
...
## 🔐 OAuth 프록시 (Mobile Only)

모바일 앱에서 `127.0.0.1` 로컬 서버 접근이 불가능하므로, 서버가 OAuth 콜백을 프록시하여 Deep Link로 앱에 전달합니다.

### GET /oauth/callback

**인증 불필요** - Google OAuth 서버에서 직접 호출

Google OAuth 인증 완료 후 리디렉트되는 엔드포인트입니다. 
인증 코드를 Deep Link URL로 변환하여 앱에 전달합니다.

**Query Parameters**:
| 파라미터 | 설명 |
|---------|------|
| code | 인증 코드 (성공 시) |
| state | CSRF 방지 state 값 |
| error | 에러 코드 (실패 시) |
| error_description | 에러 설명 |

**Response**: HTML 페이지 (Deep Link 자동 리디렉트)

```html
<!-- Deep Link로 리디렉트 -->
<script>window.location.href = 'hololive-app://callback?code=XXX&state=YYY';</script>
```

**Google Cloud Console 설정**:
Authorized redirect URIs에 추가 필요:
```
https://api.capu.blog/oauth/callback
```
